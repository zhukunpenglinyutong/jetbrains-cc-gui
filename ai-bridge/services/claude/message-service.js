...
export async function sendMessage(
  ...
  try {
    ...
    if (isCustomBaseUrl(baseUrl)) {
      ...
      console.log(
        "[DEBUG] Will use system Claude CLI (not Anthropic SDK fallback)"
      );
    }
    ...
    try {
      ...
      console.error(
        "[WARNING] Failed to change process.cwd():",
        chdirError.message
      );
    }
    ...
    if (openedFiles && typeof openedFiles === "object") {
      ...
      if (hasActive || hasOthers) {
        console.log(
          "[DEBUG] Building systemPrompt.append with active file:",
          active,
          "selection:",
          hasSelection ? "yes" : "no",
          "other files:",
        ...
        if (hasActive) {
          ...
          if (hasSelection) {
            ...
            systemPromptAppend += "\n```\n\n";
            systemPromptAppend +=
              "This selected code is the PRIMARY FOCUS. The user's question is most likely about this specific code section.\n";
          } else {
            systemPromptAppend += "\n\n";
            systemPromptAppend +=
              "This is the file the user is currently viewing/editing. It should be the main focus when answering questions.\n";
          }
        }
    ...
    console.log(
      "[PERM_DEBUG] effectivePermissionMode:",
    ...
    console.log(
      "[PERM_DEBUG] options.canUseTool:",
      options.canUseTool ? "SET" : "NOT SET"
    );
    ...
    try {
      for await (const msg of result) {
        messageCount++;
        console.log(
          `[DEBUG] Received message #${messageCount}, type: ${msg.type}`
        );
        ...
        if (msg.type === "result" && msg.is_error) {
          console.error(
            "[DEBUG] Received error result message:",
            JSON.stringify(msg)
          );
      ...
      if (loopError.spawnargs) {
        console.error(
          "[DEBUG] Error spawnargs:",
          JSON.stringify(loopError.spawnargs)
        );
      }
    ...
    console.log(
      `[DEBUG] Message loop completed. Total messages: ${messageCount}`
    );
 ...
 */
export async function sendMessageWithAnthropicSDK(
  message,
  resumeSessionId,
  cwd,
  permissionMode,
  ...
  try {
    ...
    console.log(
      "[DEBUG] Using Anthropic SDK fallback for custom Base URL (non-streaming)"
    );
    ...
    if (resumeSessionId && resumeSessionId !== "") {
      ...
      if (historyMessages.length > 0) {
        ...
        console.log(
          ...
          "history messages for session continuity"
        );
      }
    }
    ...
    if (response.error || response.type === "error") {
      const errorMsg =
        response.error?.message || response.message || "Unknown API error";
...
export async function sendMessageWithAttachments(
  ...
  try {
    ...
    if (openedFiles && typeof openedFiles === "object") {
      ...
      if (hasActive || hasOthers) {
        console.log(
          "[DEBUG] (withAttachments) Building systemPrompt.append with active file:",
          active,
          "selection:",
          hasSelection ? "yes" : "no",
          "other files:",
    ...
    console.log(
      "[DEBUG] (withAttachments) Using SDK built-in Claude CLI (cli.js)"
    );
    ...
    console.log(
      "[PERM_DEBUG] (withAttachments) permissionMode:",
      permissionMode
    );
    console.log(
      "[PERM_DEBUG] (withAttachments) normalizedPermissionMode:",
    ...
    const preToolUseHook = async (input) => {
      console.log(
        "[PERM_DEBUG] (withAttachments) PreToolUse hook called:",
        input.tool_name
      );
      ...
      if (normalizedPermissionMode !== "default") {
        console.log(
          "[PERM_DEBUG] (withAttachments) Auto-approve (non-default mode)"
        );
      ...
      try {
        ...
        console.log(
          "[PERM_DEBUG] (withAttachments) canUseTool returned:",
          result.behavior
        );
        ...
        console.error(
          "[PERM_DEBUG] (withAttachments) canUseTool error:",
          error.message
        );
        return {
          decision: "block",
    ...
    const options = {
      ...
      canUseTool:
        normalizedPermissionMode === "default" ? canUseTool : undefined,
      hooks:
    ...
    console.log(
      "[PERM_DEBUG] (withAttachments) options.canUseTool:",
      options.canUseTool ? "SET" : "NOT SET"
    );
    console.log(
      "[PERM_DEBUG] (withAttachments) options.hooks:",
      options.hooks ? "SET (PreToolUse)" : "NOT SET"
    );
    console.log(
      "[PERM_DEBUG] (withAttachments) options.permissionMode:",
    ...
    try {
      for await (const msg of result) {
          ...
          if (Array.isArray(content)) {
            for (const block of content) {
              if (block.type === "text") {
                ...
                console.log(
                  "[DEBUG] Tool use payload (withAttachments):",
                  JSON.stringify(block)
                );
              } else if (block.type === "tool_result") {
                console.log(
                  "[DEBUG] Tool result payload (withAttachments):",
                  JSON.stringify(block)
                );
              }
            }
        ...
        if (msg.type === "result" && msg.is_error) {
          console.error(
            "[DEBUG] (withAttachments) Received error result message:",
            JSON.stringify(msg)
          );
          const errorText = msg.result || msg.message || "API request failed";
          throw new Error(errorText);
      ...
      // 捕获 for await 循环中的错误
      console.error(
        "[DEBUG] Error in message loop (withAttachments):",
        loopError.message
      );
      console.error("[DEBUG] Error name:", loopError.name);
      console.error("[DEBUG] Error stack:", loopError.stack);
