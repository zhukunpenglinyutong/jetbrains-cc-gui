...
const GenericToolBlock = memo(({ name, input, result }: GenericToolBlockProps) => {
  ...
  const isCollapsible = ['grep', 'glob', 'write', 'save-file', 'askuserquestion', 'update_plan', 'shell_command', 'exitplanmode', 'websearch', 'webfetch', 'skill', 'useskill', 'runskill', 'run_skill', 'execute_skill'].includes(lowerName) || isMcpTool;
