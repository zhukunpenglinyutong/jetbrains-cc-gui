...
const LABELS: Record<string, string> = {
  ...
  'modes.sandbox.label': 'Sandbox Mode',
  'modes.sandbox.shortLabel': 'Sandbox',
...
// Mirrors the real hook: empty payload falls back to the static role models
// until listModels resolves ("before roles load").
...
describe('ModeSelect rendering and selection', () => {
  it('renders OMP model-role labels from the ompModes i18n keys', () => {
    render(<ModeSelect value="default" onChange={vi.fn()} provider="omp" />);
    fireEvent.click(screen.getByRole('button'));
  ...
  it('translates the gemini sandbox posture labels', () => {
    render(<ModeSelect value="sandbox" onChange={vi.fn()} provider="gemini" />);

    expect(screen.getByRole('button').textContent).toContain('Sandbox');
    expect(screen.getByRole('button').textContent).not.toContain('Sandbox Mode');
