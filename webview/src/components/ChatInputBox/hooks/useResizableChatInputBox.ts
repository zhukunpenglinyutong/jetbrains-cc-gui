...
/**
 * Resizable input box constraints/heuristics.
 *
 * Notes:
 * - Default mins are "nice" sizes; we dynamically clamp them to the current container/window bounds
 *   so the input is never forced larger than the available space.
 * - Wrapper height controls the scroll area of the contenteditable container.
 */
...
export function useResizableChatInputBox({
  ...
  const [size, setSize] = useState<SizeState>(() => {
    try {
      ...
      debugWarn('[useResizableChatInputBox] Failed to load persisted size:', e);
  ...
  useEffect(() => {
    try {
      ...
      debugWarn('[useResizableChatInputBox] Failed to persist size:', e);
    }
  }, [size]);
