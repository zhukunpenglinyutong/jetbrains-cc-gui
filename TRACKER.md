...
| B-029 | Message Rendering / Streaming | Markdown-Tabellen werden während des Streamings nicht gerendert — Pipe-Zeichen, Bindestriche und Zeilenumbrüche erscheinen als Roh-Text. Erst nach Stream-Ende wird die Tabelle korrekt als HTML-Table dargestellt. Bei langen Streams mit umfangreichen Tabellen (z.B. Änderungspläne) ist der Inhalt während des Streamings kaum lesbar. **Fix:** Tabellen-Parser in `renderStreamingContent` — erkennt Pipe-Zeilen + Separator, rendert als HTML-Table sobald Tabelle vollständig (nächste Nicht-Pipe-Zeile folgt). Kein Column-Flicker während Streaming. | fixed | 2026-03-01 | jh.13 | — | `release/jh.13` |
...
| B-037 | Message Rendering / Streaming | **Layout-Shift am Stream-Ende — Content ruckelt nach unten.** Wenn das Streaming endet, springt der sichtbare Bereich abrupt nach unten. **Root cause:** Gleichzeitig passieren drei Dinge: (1) `MarkdownBlock` wechselt von `renderStreamingContent` (leichtgewichtig) zu `marked.parse` (voll), was die DOM-Struktur ändert (Code-Block-Wrapper, Copy-Buttons). (2) Ein doppeltes `requestAnimationFrame` + `innerHTML`-Ersetzung erzeugt einen sichtbaren Flash (`MarkdownBlock.tsx:448-482`). (3) Spinner/StatusPanel-Elemente verschwinden und geben Platz frei. Der kumulative Effekt ist ein spürbarer Ruck. **Verwandt:** B-016, B-020. | open | 2026-03-23 | — | — | — |
| B-034 | Message Rendering / Streaming | **Assistant-Nachrichten verschwinden beim Absenden eines Prompts in bestehender Session.** Race Condition zwischen `onStreamStart` und `messages_update`. In jh.11 gefixt via `streamingOriginalIdxRef` Guard. **v0.3.1:** Neuer Fix via Index-Vergleich in `messageCallbacks.ts` — `__turnId`-Guard verhindert falsches Stamping, `ensureStreamingAssistantInList` preserviert Placeholder. Test in `useWindowCallbacks.test.ts`. **Verwandt:** B-021, B-028. | fixed | 2026-03-23 | jh.13 | — | `release/jh.13` |
| B-038 | Message Rendering / CSS | **Mauszeiger bleibt I-beam über Scrollbalken.** `cursor: text` auf `.messages-container` vererbt sich an alle Scrollbalken (Chat-Container + Tabellen-Wrapper). Scrollbalken zeigen I-beam statt Default-Pfeil. **Fix:** `cursor: text` von `.messages-container` nach `.markdown-content` verschoben — Text behält I-beam, Scrollbalken zeigen Pfeil. | fixed | 2026-03-23 | jh.13 | — | `release/jh.13` |
...
| F-014 | Settings / Tabs | **Tab Status Indicator Toggle** — "..." (Streaming) und "(Completed)" im Tab-Titel ein-/ausschaltbar über Settings → Behavior. Persistiert via `PropertiesComponent` (application-level). Default: aktiviert. Pipeline: BehaviorTab toggle → `sendToJava` → `SettingsHandler` → `ProjectConfigHandler` → `PropertiesComponent`. `ChatWindowDelegate.updateTabStatus()` liest den Wert direkt bei jedem Status-Update. | fixed | 2026-03-23 | jh.13 | — | `release/jh.13` |
