...
public class EditorContextCollector {
    ...
    private final WorkspaceContextCollector workspaceCollector = new WorkspaceContextCollector();
    ...
    private JsonObject buildContextJson(Editor editor, String activeFile) {
        ...
        if (workspaceContextEnabled) {
            try {
                JsonObject workspaceContext = this.workspaceCollector.collectWorkspaceContext(project);
                if (workspaceContext != null && workspaceContext.size() > 0) {
                    ...
                    if (activeFile != null && workspaceContext.has("isWorkspace")
                        ...
                        String subproject = this.workspaceCollector.getSubprojectForFile(project, activeFile);
                        if (subproject != null) {
