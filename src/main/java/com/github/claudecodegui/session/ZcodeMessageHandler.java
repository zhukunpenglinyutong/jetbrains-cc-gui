...
/**
 * ZCode message callback handler (mirrors GrokMessageHandler).
 *
 ...
 *
 * Protocol tags from the ai-bridge ZCode normalizer:
 ...
 */
public class ZcodeMessageHandler implements MessageCallback {
    private static final Logger LOG = Logger.getInstance(ZcodeMessageHandler.class);
    ...
    public ZcodeMessageHandler(SessionState state, CallbackHandler callbackHandler) {
        this.state = state;
    ...
    public void onMessage(String type, String content) {
        LOG.debug("ZcodeMessageHandler.onMessage: type=" + type);
        ...
        switch (type) {
            ...
            case "message_end":
                ...
                break;
            default:
                LOG.debug("ZcodeMessageHandler: Unhandled message type: " + type);
        }
    }
    ...
    public void onComplete(SDKResult result) {
        ...
        if (wasStreaming && !streamEndedBeforeComplete) {
            LOG.warn("ZCode onComplete called without prior stream_end; forcing stream cleanup");
            callbackHandler.notifyMessageUpdate(state.getMessages());
            callbackHandler.notifyStreamEnd();
        }
    ...
    private void handleAssistantMessage(String jsonContent) {
        try {
            ...
            // Preserve any usage already stamped by [USAGE] — a final [MESSAGE] from the
            // normalizer may have no usage and would wipe the context-ring snapshot.
            JsonObject priorUsage = extractUsageFromAssistantRaw(target.raw);
            // Always merge into existing raw so a final text-only [MESSAGE] never
            // replaces tool_use blocks captured earlier in the turn.
            if (target.raw != null) {
                target.raw = messageMerger.mergeAssistantMessage(target.raw, msgJson);
            } else {
                target.raw = parsed.raw;
            }
            // Prefer usage from the incoming message; otherwise restore the prior
            // snapshot so the next usage refresh still works.
            ...
            LOG.warn("Failed to parse ZCode assistant message: " + e.getMessage());
        }
    }
    ...
    private void handleUserMessage(String jsonContent) {
        try {
            ...
            // Live user text is owned by SessionSendService at send-time. User echoes
            ...
            if (userText == null || userText.isEmpty()) {
                LOG.debug("ZCode user message has no text; skipping");
                return;
            }
            ...
            for (int i = messages.size() - 1; i >= 0; i--) {
                ...
                if (userText.equals(msg.content)) {
                    ...
                    LOG.debug("ZCode user message matched existing send-time bubble; not duplicating");
                    callbackHandler.notifyMessageUpdate(state.getMessages());
                    return;
                }
            }
            ...
            LOG.debug("ZCode user message with no matching state entry; not adding to avoid duplicate bubble");
        } catch (Exception e) {
            LOG.warn("Failed to parse ZCode user message: " + e.getMessage());
        }
    }
    ...
    private void handleResultMessage(String jsonContent) {
        ...
        try {
            ...
            LOG.debug("ZCode result parse skipped: " + e.getMessage());
        }
    }
    ...
    private void handleSessionId(String id) {
        if (id != null && !id.trim().isEmpty()) {
            ...
            LOG.info("Captured ZCode session ID: " + id.trim());
        }
    }
    ...
    private void handleToolResult(String content) {
        ...
        try {
            ...
            LOG.warn("Failed to parse ZCode tool_result: " + e.getMessage());
        }
    }
    ...
    private void handleUsage(String content) {
        ...
        try {
            ...
            ensureAssistantRaw();
            // Prefer snake_case for stored message.usage (OpenAI shape); camelCase is fallback input.
            JsonObject canonical = com.github.claudecodegui.provider.zcode.ZcodeContextUsageBuilder
            ...
            int used = com.github.claudecodegui.provider.zcode.ZcodeContextUsageBuilder.extractUsedTokens(storedUsage);
            if (used > 0) {
                int maxTokens = com.github.claudecodegui.handler.provider.ModelProviderHandler
                        .getModelContextLimit(state.getModel());
                LOG.info("ZCode [USAGE] context ring: used=" + used + " max=" + maxTokens
                        + " model=" + state.getModel());
                callbackHandler.notifyUsageUpdate(used, maxTokens);
            } else {
                LOG.warn("ZCode [USAGE] received but extractUsedTokens=0 payload=" + content);
            }
            callbackHandler.notifyMessageUpdate(state.getMessages());
        } catch (Exception e) {
            LOG.debug("ZCode usage parse skipped: " + e.getMessage());
        }
    }
    ...
    private void pushContextUsageFromCurrentAssistant() {
        ...
        int used = com.github.claudecodegui.provider.zcode.ZcodeContextUsageBuilder.extractUsedTokens(usage);
        if (used <= 0) {
            return;
        }
        int maxTokens = com.github.claudecodegui.handler.provider.ModelProviderHandler
                .getModelContextLimit(state.getModel());
        LOG.info("ZCode stream_end context ring push: used=" + used + " max=" + maxTokens);
        callbackHandler.notifyUsageUpdate(used, maxTokens);
    }
