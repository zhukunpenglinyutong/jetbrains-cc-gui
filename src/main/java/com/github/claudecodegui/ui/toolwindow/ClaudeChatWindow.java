...
public class ClaudeChatWindow {
    ...
    public ClaudeChatWindow(Project project, boolean skipRegister) {
        ...
        ReasoningEffortStateService.applyToSession(this.session);
    ...
    public void restorePersistedTabSessionState(TabStateService.TabSessionState savedState) {
        ...
        // Apply saved selector state (provider/model/mode/reasoning) even when sessionId is empty.
        // This ensures restart restores the user's last choice instead of falling back to defaults.
    ...
    public synchronized void dispose() {
        ...
        // Persist the latest selector state (provider/model/mode/reasoning) before teardown.
        // This avoids restoring stale defaults on next startup for tabs that never sent messages.
        try {
            persistTabSessionState();
        } catch (Exception e) {
            LOG.debug("[TabPersist] Failed to persist tab session state during dispose: " + e.getMessage());
        }
