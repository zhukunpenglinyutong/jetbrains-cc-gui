...
/**
 * ZCode-specific daemon lifecycle owner (mirrors GrokDaemonCoordinator).
 * Dedicated instance per spec v1 (no forced sharing with Claude).
 */
class ZcodeDaemonCoordinator {
        ...
        synchronized (daemonLock) {
            ...
            try {
                ...
                if (newBridge.start()) {
                    ...
                    log.info("[ZcodeDaemonCoordinator] Daemon bridge started successfully");
                    return newBridge;
                }
                log.warn("[ZcodeDaemonCoordinator] Failed to start daemon, falling back to one-shot");
            } catch (Exception e) {
                log.debug("[ZcodeDaemonCoordinator] Daemon init failed: " + e.getMessage());
            }
            return null;
        }
    }
    ...
    void prewarmDaemonAsync(String cwd, String runtimeSessionEpoch, String sessionId) {
        ...
        prewarmFuture = CompletableFuture.runAsync(() -> {
            try {
                ...
                if (daemon == null) {
                    log.info("[ZcodeDaemonCoordinator] Prewarm skipped (daemon unavailable)");
                    return;
                }
                ...
                // ZCode credentials are resolved Node-side from the desktop client
                // config; bridge will enrich via its env config
                        ...
                        new DaemonBridge.DaemonOutputCallback() {
                            ...
                            public void onLine(String line) {
                                if (line.startsWith("[SEND_ERROR]")) {
                                    log.warn("[ZcodeDaemonCoordinator] preconnect error: " + line);
                                }
                            }

                            @Override
                            public void onStderr(String text) {
                                log.debug("[ZcodeDaemonCoordinator] preconnect stderr: " + text);
                            }

                            @Override
                            public void onError(String error) {
                                log.warn("[ZcodeDaemonCoordinator] preconnect failed: " + error);
                            }

                            @Override
                            public void onComplete(boolean success) {
                                log.info("[ZcodeDaemonCoordinator] preconnect completed: " + success);
                            }
                        }
                );

                preconnectFuture.get(45, TimeUnit.SECONDS);
                log.info("[ZcodeDaemonCoordinator] prewarm completed for epoch=" +
                        (runtimeSessionEpoch != null ? runtimeSessionEpoch : "(none)"));
            } catch (Exception e) {
                log.debug("[ZcodeDaemonCoordinator] prewarm failed: " + e.getMessage());
            }
        });
    }
    ...
    void resetPersistentRuntime(String runtimeSessionEpoch) {
        ...
        if (daemon == null || !daemon.isAlive()) {
            log.info("[ZcodeDaemonCoordinator] Skip reset; daemon unavailable for epoch=" +
                    (runtimeSessionEpoch != null ? runtimeSessionEpoch : "(none)"));
            return;
        }
        ...
        try {
                    ...
                    new DaemonBridge.DaemonOutputCallback() {
                        ...
                        public void onLine(String line) {
                            if (line != null && !line.isBlank()) {
                                log.debug("[ZcodeDaemonCoordinator] reset line: " + line);
                            }
                        }
                        ...
                        public void onStderr(String text) {
                            if (text != null && !text.isBlank()) {
                                log.debug("[ZcodeDaemonCoordinator] reset stderr: " + text);
                            }
                        }

                        @Override
                        public void onError(String error) {
                            log.warn("[ZcodeDaemonCoordinator] reset error: " + error);
                        }

                        @Override
                        public void onComplete(boolean success) {
                            log.info("[ZcodeDaemonCoordinator] reset completed: success=" + success +
            ...
            log.warn("[ZcodeDaemonCoordinator] reset failed: " + e.getMessage());
        }
    }
}
