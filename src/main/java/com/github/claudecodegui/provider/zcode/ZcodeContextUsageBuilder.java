...
/**
 * Builds a {@code ContextUsageData}-compatible payload for the ZCode provider.
 * The ZCode app-server only exposes aggregate token usage (not Claude's category
 * breakdown), so we synthesize: used conversation tokens + free space within the
 * model limit.
 */
public final class ZcodeContextUsageBuilder {

    private ZcodeContextUsageBuilder() {
    }
    ...
    public static JsonObject build(int usedTokens, int maxTokens, String model) {
        ...
        root.addProperty("source", "zcode-synthesized");
     ...
     * The ZCode app-server already emits snake_case; camelCase is mapped here as fallback.
     *
    ...
    /**
     * Extract used-token total from a ZCode usage JSON object.
