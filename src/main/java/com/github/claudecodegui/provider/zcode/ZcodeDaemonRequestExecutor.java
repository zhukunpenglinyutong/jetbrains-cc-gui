...
/**
 * Sends ZCode requests through the long-running daemon (zcode.* NDJSON commands).
 */
class ZcodeDaemonRequestExecutor {

    private final Logger log;
    private final ZcodeSDKBridge bridge; // for processOutputLine + payload builder

    ZcodeDaemonRequestExecutor(Logger log, ZcodeSDKBridge bridge) {
        this.log = log;
        this.bridge = bridge;
    }
    ...
    CompletableFuture<SDKResult> sendMessageViaDaemon(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                log.info("[ZcodeDaemonExecutor] Sending via daemon: zcode.send");
                        ...
                        new DaemonBridge.DaemonOutputCallback() {
                            ...
                            public void onLine(String line) {
                                if (line.startsWith("[UNCAUGHT_ERROR]")
                                    ...
                                    log.warn("[ZCode Node ERROR] " + line);
                                    lastNodeError.set(line);
                                }
                                // Reuse ZcodeSDKBridge processing (Claude-shaped tags)
                            ...
                            public void onStderr(String text) {
                                ...
                                log.debug("[ZcodeDaemon:stderr] " + text);
                            }
                ...
                while (true) {
                    try {
                        ...
                        if (!daemon.isAlive()) {
                            throw new RuntimeException("ZCode daemon not alive", timeout);
                        }
                        long now = System.currentTimeMillis();
                        if (now - lastProgressLogAt >= 60_000) {
                            long elapsedSec = (now - waitStart) / 1000;
                            log.info("[ZcodeDaemonExecutor] still running (" + elapsedSec + "s)...");
                ...
                if (!hadSendError.get()) {
                    ...
                    if (result.success) {
                        ...
                        log.info("[ZcodeDaemonExecutor] aborted by user (" + elapsed + "ms)");
                        result.error = "User interrupted";
                        callback.onComplete(result);
                    } else {
                        String errorMsg = "ZCode daemon command failed";
                    ...
                    callback.onError(result.error != null ? result.error : "ZCode send error");
                }
